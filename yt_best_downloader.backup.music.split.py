...
- Enforces minimum resolution >= 1080p by default (errors if not available; use --allow-below-min to relax)
...
New:
- --split        : split audio (music mode) into segments using markers or ranges.
                   Examples:
                     --split "0:00,1:23,3:45,5:00"
                     --split "0:00-1:23=Intro,1:23-3:45=Verse,3:45-5:00=Chorus,5:00-end=Outro"
                     --split @splits.txt   (lines like above)
...
def main():
    ...
    # Post step: split audio if requested and in music mode
    if args.music and args.split:
        for info in outputs:
            vid = info.get("id")
